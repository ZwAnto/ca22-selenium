...
try:
    ...
    cursor.execute('SELECT date_record,date_value,date_desc,type,description,debit,credit,date FROM (SELECT * FROM ' + config['SQL']['TABLE'] + ' ORDER BY id DESC LIMIT 10) as t1')
    ...
    if new_md5 == md5:
        ...
        try:
            ...
            cursor.executemany('INSERT INTO ' + config['SQL']['TABLE'] + ' (date_record,date_value,date_desc,type,description,debit,credit, date) VALUES (%s, %s, %s, %s, %s, %s, %s,%s)', list(reversed(operations)))
            sql.commit()
